...
function Index(props) {
    ...
    useEffect(() =>{
        let propsValue = props.user;
        let token = localStorage.getItem('auth_token');
        if(propsValue === null && token === null){

            //meaning you are illegitimately logged in
            Swal.fire({
                title: 'Error!',
              ...
              <div className="w-full bg-indigo-100 h-screen flex flex-row flex-wrap justify-center ">
                  ...
                  <div className="w-0 md:w-1/4 lg:w-1/5 h-0 md:h-screen overflow-y-hidden bg-white shadow-lg">
                      ...
                      <div className="w-full h-screen antialiased flex flex-col hover:cursor-pointer">
                          <a className="hover:bg-gray-300 bg-gray-200 border-t-2 p-3 w-full text-xl text-left text-gray-600 font-semibold"
                             href=""><i className="fa fa-comment text-gray-600 text-2xl pr-1 pt-1 float-right"></i>Messages</a>
                          <a className="hover:bg-gray-300 bg-gray-200 border-t-2 p-3 w-full text-xl text-left text-gray-600 font-semibold"
                             href=""><i
                              className="fa fa-cog text-gray-600 text-2xl pr-1 pt-1 float-right"></i>Settings</a>
                          <button className="hover:bg-gray-300 bg-gray-200 border-t-2 p-3 w-full text-xl text-left text-gray-600 font-semibold"
                            onClick={logout}><i className="fa fa-arrow-left text-gray-600 text-2xl pr-1 pt-1 float-right"></i>Log
                              out</button>
                      </div>
                  </div>
                  <div className="w-full md:w-3/4 lg:w-4/5 p-5 md:px-12 lg:24 h-full overflow-x-scroll antialiased">
                      <div className="bg-white w-full shadow rounded-lg p-5">
                        <textarea className="bg-gray-200 w-full rounded-lg shadow border p-2" rows="5"
                          ...
                          <div className="w-full flex flex-row flex-wrap mt-3">
                              ...
                              <div className="w-2/3">
                                  <button type="button"
                                          className="float-right bg-indigo-400 hover:bg-indigo-300 text-white p-2 rounded-lg">Submit
                                  </button>
                              </div>
                          </div>
                      </div>
                      ...
                      <div className="mt-3 flex flex-col">
                          ...
                          <div className="bg-white mt-3">
                              <img className="border rounded-t-lg shadow-lg "
